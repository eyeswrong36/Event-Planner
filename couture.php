...
<head>
    <title>Event Planner - Couture Page</title>
...
<body>
    ...
    <!-- /.header -->
    <!-- page-header -->
    <div class="page-header" style="background:url('<?=$gs['Couture'];?>')center no-repeat;background-size:cover;">
        <div class="container">
            <div class="row">
                <!-- page caption -->
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 ">
                    <div class="page-caption">
                        <h1 class="page-title" style="text-shadow:3px 3px 3px black;">Couture Packages/Services</h1>
                    </div>
                </div>
                <!-- /.page caption -->
            </div>
                ...
                <div class="row">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active text-white" aria-current="page">Couture</li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                    <form class="form-row" method="get">
                        ...
                        <!-- /.price -->
                        <div class="col-xl-3 col-lg-3 col-md-3 col-sm-12 col-12 ">
                            <button class="btn btn-default btn-block" type="submit">Filter Couture</button>
                        </div>
        ...
        <div class="container">
            <div class="row">
                ...
                if(isset($_GET['search']) AND isset($_GET['filter'])){
                    ...
                    $f = $_GET['filter'];
                    if($f==0 AND $s!=""){
                        $showInfo = $con->prepare("SELECT * FROM tbl_prod WHERE prodName LIKE '%$s%' OR prodLoc LIKE '%$s%' AND prodType='2' ORDER BY rowID DESC");

                    }else if($f!=0 AND $s==""){
                        if($f==1){
                            $showInfo = $con->prepare("SELECT * FROM tbl_prod WHERE prodType='2' AND prodName LIKE '%$s%' OR prodLoc LIKE '%s%' ORDER BY prodRatings DESC");
                        }else if($f==2){
                            $showInfo = $con->prepare("SELECT * FROM tbl_prod WHERE prodType='2' AND prodName LIKE '%$s%' OR prodLoc LIKE '%s%' ORDER BY prodPrice DESC");
                        }else if($f==3){
                            $showInfo = $con->prepare("SELECT * FROM tbl_prod WHERE prodType='2' AND prodName LIKE '%$s%' OR prodLoc LIKE '%s%' ORDER BY prodPrice ASC");
                        }
                        

                    }else if($f!=0 AND $s!=""){
                        if($f==1){
                            $showInfo = $con->prepare("SELECT * FROM tbl_prod WHERE prodName LIKE '%$s%' OR prodLoc LIKE '%$s%' AND prodType='2' ORDER BY prodRatings DESC");
                        }else if($f==2){
                            $showInfo = $con->prepare("SELECT * FROM tbl_prod WHERE prodName LIKE '%$s%' OR prodLoc LIKE '%$s%' AND prodType='2' ORDER BY prodPrice DESC");
                        }else if($f==3){
                            $showInfo = $con->prepare("SELECT * FROM tbl_prod WHERE prodName LIKE '%$s%' OR prodLoc LIKE '%$s%' AND prodType='2' ORDER BY prodPrice ASC");
                        }
                        
                    }
                }else{
                    $showInfo = $con->prepare("SELECT * FROM tbl_prod WHERE prodType='2' ORDER BY rowID DESC");
                }
                ...
                <div class="row">
                    <div class="col-md-12">
                        <h3>Suggested Photographers</h3>
                    </div>
                    <?php 
                    $showCat = $con->prepare("SELECT * FROM tbl_prod WHERE prodType='3' ORDER BY rowID DESC LIMIT 3");
                    $showCat->execute();
